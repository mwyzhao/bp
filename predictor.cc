...
void InitPredictor_openend() {
  ...
  cout << "Predictor size: " << get_mem_usage() << " bytes" << endl;
    ...
    if(global_table[i][index].tag == tag){
      ...
      if(pred_made){
        ...
        if(global_table[pred_table][pred_index].u == 0 &&
           (global_table[pred_table][pred_index].ctr == get_weak_not_taken(GCTR_SIZE) ||
           global_table[pred_table][pred_index].ctr == get_weak_taken(GCTR_SIZE)) &&
...
void UpdatePredictor_openend(UINT32 PC, bool resolveDir, bool predDir, UINT32 branchTarget) {
  ...
  // Update base table
  if(resolveDir == TAKEN){
    base_table[PC & get_mask(BTI_SIZE)].ctr = sat_ctr_inc(base_table[PC & get_mask(BTI_SIZE)].ctr, BCTR_SIZE);
  }
  else{
    base_table[PC & get_mask(BTI_SIZE)].ctr = sat_ctr_dec(base_table[PC & get_mask(BTI_SIZE)].ctr, BCTR_SIZE);
  }
  ...
  if(pred_table != 0 &&
     ...
     global_table[pred_table][pred_index].u == 0 &&
     (global_table[pred_table][pred_index].ctr == get_weak_not_taken(GCTR_SIZE) ||
     global_table[pred_table][pred_index].ctr == get_weak_taken(GCTR_SIZE))){ // consider checking for all the other cases from above
    if(altpred == resolveDir){
      use_alt_on_na = sat_ctr_inc(use_alt_on_na, UAON_SIZE);
    }
    else{
      use_alt_on_na = sat_ctr_dec(use_alt_on_na, UAON_SIZE);
    }
  }
  // Update useful counter
  if(pred_table != 0 && pred != altpred){
    if(pred == resolveDir){
      global_table[pred_table][pred_index].u = sat_ctr_inc(global_table[pred_table][pred_index].u, U_SIZE);
    }
    else{
      global_table[pred_table][pred_index].u = sat_ctr_dec(global_table[pred_table][pred_index].u, U_SIZE);
    }
  }
  // "Gracefully" reset u
  ...
  if(pred_table != 0){
    if(resolveDir == TAKEN){
      global_table[pred_table][pred_index].ctr = sat_ctr_inc(global_table[pred_table][pred_index].ctr, GCTR_SIZE);
    }
    else{
      global_table[pred_table][pred_index].ctr = sat_ctr_dec(global_table[pred_table][pred_index].ctr, GCTR_SIZE);
    }
  }
  // Allocate new counters
  if(pred_table < N_TABLES - 1){ // prediction did not come from longest history table
    // find first free index of tables with longer history
    // int tables = N_TABLES - 1 - pred_table;
    ...
    // if no free indexes found, decrement all u
